...
| Commit    | What                                                                                                                                                                                                                                                  |
| --------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `(M6)`    | feat(board): stopzone element (🛑/🚦/📍) — dwell-to-satisfy, stall on running a sign, goal gated until all zones visited (zones double as route checkpoints); figure-8 gets signs; new junction-drill board + grade-4 challenge; toolbox XML escaping |
| `(M5)`    | feat(challenges): challenge ladder — 8 missions across the grades, 1-3 star scoring from run records (presses/time/bonus), Challenges tab, stars in the goal overlay                                                                                  |
...
Snapshot after this session: **313 tests (292 unit + 21 browser)**, strict TS 0 errors, lint/format clean. Quality gates: Security APPROVED (no conditions); Code Review REPAIR → fixed in `07bc9a9`.
